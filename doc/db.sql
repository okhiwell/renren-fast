...
-- 性能测试用例表
CREATE TABLE if not exists `test_stress_case` (
...
-- 性能测试用例文件表
CREATE TABLE if not exists `test_stress_case_file` (
...
-- 性能测试报告文件表
CREATE TABLE if not exists `test_stress_case_reports` (
...
-- 性能测试分布式节点表
CREATE TABLE if not exists `test_stress_slave` (
...
-- Grafana监控视图
INSERT INTO `sys_menu` (`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES ('36', '31', 'Grafana监控视图', 'modules/test/smallTools.html', 'test:stress', '1', 'fa fa-clipboard', '5');
...
CREATE TABLE if not exists `test_debug_case_reports` (
  `report_id` bigint NOT NULL AUTO_INCREMENT,
  `case_id` bigint NOT NULL COMMENT '所关联的用例',
  `file_id` bigint NOT NULL COMMENT '所关联的用例文件',
  `origin_name` varchar(200) NOT NULL COMMENT '测试报告名称',
...
CREATE TABLE if not exists `test_stress_thread_set` (
